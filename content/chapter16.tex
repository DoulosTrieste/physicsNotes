...
\begin{document}
	\section{Static Electricity}
		...
		Field lines are the path a test charge would take within that electric field. The closer the field lines are, the stronger the electric field at that area, which means that the test charge would experience a stronger force.
		...
		\begin{center}
			\begin{tikzpicture}
			\draw (0,0) circle (1cm);
			\foreach \x in {0,1,2,3,4,5,6} \node at ({0.5*sin(60*\x)},{0.5*cos(60*\x)}) {\(-\)};
			\end{tikzpicture}
		\end{center}
